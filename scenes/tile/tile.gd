# tile.gd - Consertos, Bloqueios e Visuais de Desastre (SEM ELIF)
extends ColorRect
...
func _gui_input(event):
	if Engine.time_scale == 0: return # Bloqueio total no relatório
...
func _apagar_tile():
	...
	if estado_atual != base_bioma:
		# Se apagar, sai da lista de quebrados
...
func _aplicar_estado():
	...
	# Se desenhar por cima, repara o quebrado
	if gm_ref.trilhos_quebrados.has(Vector2i(pos_x, pos_y)): gm_ref.trilhos_quebrados.erase(Vector2i(pos_x, pos_y))
...
func _draw():
	...
	# DESASTRE: DESENHA O X VERMELHO SE QUEBRADO
